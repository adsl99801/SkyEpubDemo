...
public class HomeActivity extends Activity {
    ...
    private final int gridButtonId = 1004;
    ...
    AdapterView.OnItemClickListener onClick = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            BookInformation bi = SkyApplicationHolder.bis.get(position);
            lastBook = bi;
            openBookViewer(bi);
        }
    };
    ...
    private OnClickListener onClickListener = new OnClickListener() {
        public void onClick(View arg) {
            if (arg.getId() == libraryButtonId) {
                libraryButtonClick();
            }
            if (arg.getId() == searchButtonId) {
                searchButtonClick();
            }
            if (arg.getId() == sortButtonId) {
                sortButtonClick();
            }
            if (arg.getId() == gridButtonId) {
                gridButtonClick();
            }
            if (arg.getId() == settingButtonId) {
                settingButtonClick();
            }
            if (arg.getId() == closeSearchButtonId) {
...
//	}
    ...
    public void registerCustomFont(String fontFaceName, String fontFileName) {
        ...
        SkyApplicationHolder.customFonts.add(new CustomFont(fontFaceName, fontFileName));
    }
    ...
    public void reload(String key) {
        SkyApplicationHolder.reloadBookInformations(key);
        this.gridAdapter.reset();
        this.gridView.invalidateViews();
        this.gridView.setAdapter(gridAdapter);
    }
...
//			authorTextView.setText(bi.creator);
    ...
    class SkySortDialog extends Dialog implements OnClickListener {
        ...
        private void changeButtonColor() {
            Button button;
            if (SkyApplicationHolder.sortType == 1) {
                button = sortByTitleButton;
            } else if (SkyApplicationHolder.sortType == 2) {
                button = sortByAuthorButton;
            } else if (SkyApplicationHolder.sortType == 3) {
                button = sortByLastReadButton;
            } else {
                button = sortByDownloadButton;
            }
        ...
        public void onClick(View view) {
            ...
            if (newType != SkyApplicationHolder.sortType) {
                SkyApplicationHolder.sortType = newType;
                reload();
            }
...
//			return 120;
            ...
            if (SkyApplicationHolder.bis != null) {
                size = SkyApplicationHolder.bis.size();
            }
            return size;
        }
        ...
        public Object getItem(int position) {
            // TODO Auto-generated method stub
            return SkyApplicationHolder.bis.get(position);
        }
